...
class InputCheckable(gui.Input):
    ...
    def __init__(self, status_on=False, input_type='checkbox', *args, **kwargs):
        ...
        self.attributes[gui.Widget.EVENT_ONCHANGE] = \
            ...
            {'emitter_identifier': str(
                self.identifier), 'event_name': gui.Widget.EVENT_ONCHANGE}
...
class RadioButton(InputCheckable):
    ...
    def __init__(self, status_on=False, group='group', *args, **kwargs):
        ...
        super(RadioButton, self).__init__(
            status_on, input_type='radio', *args, **kwargs)
...
class SkyWeatherConfigure(App):
    def __init__(self, *args):
        ...
        super(SkyWeatherConfigure, self).__init__(
    ...
    def readJSON(self):
        ...
        if os.path.isfile('SkyWeather2.JSON'):
            ...
            with open('SkyWeather2.JSON') as json_file:
                ...
                self.enable_MySQL_Logging = self.getJSONValue(
                ...
                self.enable_WLAN_Detection = self.getJSONValue(
                    'enable_WLAN_Detection')
                self.PingableRouterAddress = self.getJSONValue(
                    'PingableRouterAddress')
                ...
                self.DustSensorPowerPin = self.getJSONValue(
                    'DustSensorPowerPin')
                self.GPIO_Pin_PowerDrive_Sig1 = self.getJSONValue(
                    'GPIO_Pin_PowerDrive_Sig1')
                self.GPIO_Pin_PowerDrive_Sig2 = self.getJSONValue(
                    'GPIO_Pin_PowerDrive_Sig2')
                self.WATCHDOGTRIGGER = self.getJSONValue('WATCHDOGTRIGGER')
                self.Camera_Night_Enable = self.getJSONValue(
                    'Camera_Night_Enable')
    ...
    def buildScreen1(self):
        ...
        P6NTheader = gui.Label(
            "English or Metric Units", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P6NTheader, 'P6NTheader')

        self.F_English_Metric = gui.CheckBoxLabel(
            'English (not checked) or Metric (checked)', self.English_Metric, height=30, style='margin:5px; background: LightGray ')
    ...
    def buildScreen2(self):
        ...
        vbox = VBox(width=600, height=510,
                    style="background: LightGray; border: 5px solid red")
        ...
        # screen
        screenheader = gui.Label(
            "Main and Text Notification Tab", style='margin:10px')
        ...
        plabel = gui.Label(
            "Mail User", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(plabel, 'plabel')

        self.F_mailUser = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_mailUser.set_value(self.mailUser)
        vbox.append(self.F_mailUser, 'mailUser')

        p1label = gui.Label(
            "Mail Password", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p1label, 'p1label')

        self.F_mailPassword = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_mailPassword.set_value(self.mailPassword)
        vbox.append(self.F_mailPassword, 'mailPassword')

        p3label = gui.Label(
            "Notify Address", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p3label, 'p3label')

        self.F_notifyAddress = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_notifyAddress.set_value(self.notifyAddress)
        vbox.append(self.F_notifyAddress, 'notifyAddress')

        p4label = gui.Label(
            "From Address", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p4label, 'p4label')

        self.F_fromAddress = gui.TextInput(
            width=300, height=30, style="margin:5px")
        ...
        self.F_enableText = gui.CheckBoxLabel(
            'enable Text Messaging', self.SWDEBUG, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_enableText, 'self.F_enableText')

        p5label = gui.Label(
            "Text Notify Address", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
    ...
    def buildScreen3(self):
        ...
        self.F_runLEDs = gui.CheckBoxLabel(
            'Enable Pixel/NeoPixel', self.runLEDs, height=30, style='margin:5px; background: LightGray ')
        ...
        self.F_SolarMAX_Present = gui.CheckBoxLabel(
            'SolarMAX Present', self.SolarMAX_Present, height=30, style='margin:5px; background: LightGray ')
        ...
        self.F_BMP280_Altitude_Meters = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_BMP280_Altitude_Meters.set_value(
            str(self.BMP280_Altitude_Meters))
        vbox.append(self.F_BMP280_Altitude_Meters, 'BMP280_Altitude_Meters')

        P3Nheader = gui.Label(
            "Sunlight Gain", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
    ...
    def buildScreen4(self):
        ...
        # screen
        screen1header = gui.Label(
            "WeatherSTEM / WeatherUnderGround Configuration Tab", style='margin:10px')
        vbox.append(screen1header)
        ...
        self.F_USEWEATHERSTEM = gui.CheckBoxLabel(
            'Enable WeatherSTEM', self.USEWEATHERSTEM, height=30, style='margin:5px; background: LightGray ')
        ...
        vbox.append(self.F_INTERVAL_CAM_PICS__SECONDS,
                    'INTERVAL_CAM_PICS__SECONDS')

        p6label = gui.Label("SkyWeather Station Key",
                            style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p6label, 'p6label')

        self.F_STATIONKEY = gui.TextInput(
            width=300, height=30, style="margin:5px")
        ...
        self.F_WeatherUnderground_Present = gui.CheckBoxLabel(
            'Enable WeatherUnderground', self.WeatherUnderground_Present, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_WeatherUnderground_Present,
                    'self.F_WeatherUnderground_Present')

        p7label = gui.Label(
            "Station ID", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p7label, 'p7label')

        self.F_WeatherUnderground_StationID = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_WeatherUnderground_StationID.set_value(
            self.WeatherUnderground_StationID)
        vbox.append(self.F_WeatherUnderground_StationID,
                    'WeatherUnderground_StationID')

        p8label = gui.Label(
            "Station Key", style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        ...
        vbox.append(self.F_WeatherUnderground_StationKey,
                    'WeatherUnderground_StationKey')
    ...
    def buildScreen5(self):
        ...
        # screen 1
        screen1header = gui.Label(
            "Blynk / WeatherSense Tab", style='margin:10px')
        vbox.append(screen1header)

        P5Nheader = gui.Label(
            "Blynk Configuration", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P5Nheader, 'P5Nheader')

        self.F_USEBLYNK = gui.CheckBoxLabel(
            'Enable Blynk', self.USEBLYNK, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEBLYNK, 'self.F_USEBLYNK')

        p8label = gui.Label("Blynk App Authorization",
                            style='position:absolute; left:5px; top:40px;'+self.labelstyle)
        vbox.append(p8label, 'p8label')

        self.F_BLYNK_AUTH = gui.TextInput(
            width=300, height=30, style="margin:5px")
        self.F_BLYNK_AUTH.set_value(self.BLYNK_AUTH)
        vbox.append(self.F_BLYNK_AUTH, 'BLYNK_AUTH')
        #
        P1Nheader = gui.Label(
            "WeatherSense Sensors", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P1Nheader, 'P1Nheader')
        self.F_USEWSLIGHTNING = gui.CheckBoxLabel(
            'Use WS Lightning ', self.USEWSLIGHTNING, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSLIGHTNING, 'self.F_USEWSLIGHTNING')
        self.F_USEWSAQI = gui.CheckBoxLabel(
            'Use WS Air Quality ', self.USEWSAQI, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSAQI, 'self.F_USEWSAQI')
        self.F_USEWSSKYCAM = gui.CheckBoxLabel(
            'Use WS Sky Camera ', self.USEWSSKYCAM, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_USEWSSKYCAM, 'self.F_USEWSSKYCAM')
        self.F_ALTWSSKYCAM = gui.CheckBoxLabel(
            'Alternate SkyCam/WS SkyCamera ', self.ALTWSSKYCAM, height=30, style='margin:5px; background: LightGray ')
    ...
    def buildScreen6(self):
        ...
        P5Nheader = gui.Label(
            "Pin Configurations", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P5Nheader, 'P5Nheader')

        p8label = gui.Label(
            "Dust Sensor SCL Pin", style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p8label, 'p8label')

        self.F_DustSensorSCL = gui.TextInput(
        ...
        p9label = gui.Label(
            "Dust Sensor SDA Pin", style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p9label, 'p9label')

        self.F_DustSensorSDA = gui.TextInput(
            width=200, height=30, style="margin:5px")
        self.F_DustSensorSDA.set_value(str(self.DustSensorSDA))
        vbox.append(self.F_DustSensorSDA, 'DustSensorSDA')

        p1label = gui.Label("Dust Sensor Power Pin",
                            style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p1label, 'p1label')

        self.F_DustSensorPowerPin = gui.TextInput(
            width=200, height=30, style="margin:5px")
        self.F_DustSensorPowerPin.set_value(str(self.DustSensorPowerPin))
        vbox.append(self.F_DustSensorPowerPin, 'DustSensorPowerPin')

        p2label = gui.Label("GPIO Fan Power Pin Sig1",
                            style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p2label, 'p2label')

        self.F_GPIO_Pin_PowerDrive_Sig1 = gui.TextInput(
            width=200, height=30, style="margin:5px")
        self.F_GPIO_Pin_PowerDrive_Sig1.set_value(
            str(self.GPIO_Pin_PowerDrive_Sig1))
        vbox.append(self.F_GPIO_Pin_PowerDrive_Sig1,
                    'GPIO_Pin_PowerDrive_Sig1')

        p3label = gui.Label("GPIO Fan Power Pin Sig2",
                            style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p3label, 'p3label')

        self.F_GPIO_Pin_PowerDrive_Sig2 = gui.TextInput(
            width=200, height=30, style="margin:5px")
        self.F_GPIO_Pin_PowerDrive_Sig2.set_value(
            str(self.GPIO_Pin_PowerDrive_Sig2))
        vbox.append(self.F_GPIO_Pin_PowerDrive_Sig2,
                    'GPIO_Pin_PowerDrive_Sig2')

        p4label = gui.Label(
            "WatchDog Trigger Pin", style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p4label, 'p4label')

        self.F_WATCHDOGTRIGGER = gui.TextInput(
            width=200, height=30, style="margin:5px")
    ...
    def buildScreen7(self):
        ...
        P7Nheader = gui.Label(
            "REST Interface", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P7Nheader, 'P7Nheader')
        self.F_REST_Enable = gui.CheckBoxLabel(
            'REST Enable', self.REST_Enable, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_REST_Enable, 'self.F_REST_Enable')

        P6Nheader = gui.Label(
            "MQTT Configuration", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P6Nheader, 'P6Nheader')

        self.F_MQTT_Enable = gui.CheckBoxLabel(
            'MQTT Enable', self.MQTT_Enable, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_MQTT_Enable, 'self.F_MQTT_Enable')

        p4label = gui.Label(
            "MQTT Server URL ", style='position:absolute; left:5px; top:40px;'+shortlabelstyle)
        vbox.append(p4label, 'p4label')

        self.F_MQTT_Server_URL = gui.TextInput(
            width=200, height=30, style="margin:5px")
    ...
    def buildScreen8(self):
        ...
        P5Nheader = gui.Label(
            "Night Camera Enable", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P5Nheader, 'P5Nheader')
        self.F_Camera_Night_Enable = gui.CheckBoxLabel(
            'Night Vision Enable', self.Camera_Night_Enable, height=30, style='margin:5px; background: LightGray ')
        vbox.append(self.F_Camera_Night_Enable, 'self.F_Camera_Night_Enable')

        P5Nheader = gui.Label(
            "Camera Rotation", style='position:absolute; left:5px; top:30px;'+self.headerstyle)
        vbox.append(P5Nheader, 'P5Nheader')
...
###################
    ...
    def main(self):
        ...
        self.mainContainer = Container(
            width=widthBox, height=heightBox, margin='0px auto', style="position: relative")
        ...
        header = gui.Label("SkyWeather2 Configuration Tool",
                           style='position:absolute; left:5px; top:30px')
        version = gui.Label(
            "Version 003", style='position:absolute; left:5px; top:50px')
        # bottom buttons

        cancel = gui.Button(
            'Cancel', style='position:absolute; left:550px; height: 30px; width:100px; margin:10px; top:5px')
        cancel.onclick.do(self.onCancel)
        save = gui.Button(
            'Save', style='position:absolute; left:400px; height: 30px; width:100px;  margin: 10px;  top:5px')
        save.onclick.do(self.onSave)
        exit = gui.Button(
            'Save and Exit', style='position:absolute; left:475px; height: 30px; width:100px;  margin: 10px;  top:95px')
        exit.onclick.do(self.onExit)
        reset = gui.Button(
            'Reset to Defaults', style='position:absolute; left:400px;height: 30px;   width:250px; margin: 10px; top:50px')
...
configuration = {'config_enable_file_cache': True, 'config_multiple_instance': True, 'config_port': 8001,
                 'config_address': '0.0.0.0', 'config_start_browser': False, 'config_project_name': 'untitled', 'config_resourcepath': './res/'}
