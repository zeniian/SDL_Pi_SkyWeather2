...
def sendWeatherUndergroundData(Rain24Hour):
    ...
    if ((config.WeatherUnderground_Present == True) and (state.lastMainReading != "Never")):
        ...
        try:
            ...
            dewpoint = state.OutdoorTemperature - \
                ((100.0 - state.OutdoorHumidity) / 5.0)
            ...
            myURL += "&indoortempf=%0.2f" % (
                (state.IndoorTemperature*9.0/5.0)+32.0)
            myURL += "&indoorhumidity%0.2f=" % state.IndoorHumidity
            myURL += "&solarradiation=%0.2f" % (
                float(state.SunlightVisible)*0.0079)
            ...
            # send it
            r = requests.get(
                "https://rtupdate.wunderground.com/weatherstation/updateweatherstation.php", params=myURL)
