...
def readJSON(addPath):
    ...
    if os.path.isfile(addPath+'SkyWeather2.JSON'):
        ...
        with open(addPath+'SkyWeather2.JSON') as json_file:
            ...
            config.enable_WLAN_Detection = getJSONValue(
                'enable_WLAN_Detection')
            config.PingableRouterAddress = getJSONValue(
            ...
            config.BMP280_Altitude_Meters = float(
                getJSONValue('BMP280_Altitude_Meters'))
            config.Sunlight_Gain = getJSONValue('Sunlight_Gain')
            config.USEWEATHERSTEM = getJSONValue('USEWEATHERSTEM')
            config.INTERVAL_CAM_PICS__SECONDS = int(
                getJSONValue('INTERVAL_CAM_PICS__SECONDS'))
            ...
            config.GPIO_Pin_PowerDrive_Sig1 = int(
                getJSONValue('GPIO_Pin_PowerDrive_Sig1'))
            config.GPIO_Pin_PowerDrive_Sig2 = int(
                getJSONValue('GPIO_Pin_PowerDrive_Sig2'))
