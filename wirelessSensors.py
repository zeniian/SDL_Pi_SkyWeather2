...
def processFT020T(sLine, lastFT020TTimeStamp):
    ...
    if (wTemp > 140.0):
        ...
        if (config.SWDEBUG):
            ...
            sys.stdout.write(
                'This is the raw temperature: ' + str(wTemp) + '\n')
...
def processF016TH(sLine):
    ...
    indoorTH.addITReading(var["device"], var["channel"], state.IndoorTemperature,
                          var["humidity"], var["battery"],  var["time"])
    ...
    if (config.enable_MySQL_Logging == True):
        ...
        try:
            ...
            batteryPower = float(state["batterycurrent"]) * \
                float(state["batteryvoltage"])/1000.0
            loadPower = float(state["loadcurrent"]) * \
                float(state["loadvoltage"])/1000.0
            solarPower = float(state["solarpanelcurrent"]) * \
                float(state["solarpanelvoltage"])/1000.0
            batteryCharge = 0.0
    ...
    if ((myProtocol == 8) or (myProtocol == 10) or (myProtocol == 11)):
        ...
        if (config.enable_MySQL_Logging == True):
            ...
            try:
                ...
                cur = con.cursor()
                batteryPower = float(
                    myState["batterycurrent"]) * float(myState["batteryvoltage"])/1000.0
                loadPower = float(myState["loadcurrent"]) * \
                    float(myState["loadvoltage"])/1000.0
                solarPower = float(
                    myState["solarpanelcurrent"]) * float(myState["solarpanelvoltage"])/1000.0
                batteryCharge = 0.0
                ...
                state.SolarMaxInsideTemperature = float(
                    myState["internaltemperature"])
                state.SolarMaxInsideHumidity = float(
                    myState["internalhumidity"])
    ...
    if (config.enable_MySQL_Logging == True):
        ...
        try:
            ...
            values = "%d, %d, %d, %d, %d,%6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f, %6.2f,%6.2f,%6.2f, %d, %d, %d, %d,%d,%6.2f, %6.2f, %6.2f,\'%s\', \'%s\'" % (
                ...
                state['instanteq_pga'],
                state["batteryvoltage"], state["batterycurrent"], state["loadvoltage"], state["loadcurrent"],
                state["solarpanelvoltage"], state["solarpanelcurrent"], state["auxa"], state["solarpresent"], state[
                    "aftershockpresent"], state["keepalivemessage"], state["lowbattery"],     batteryCharge, state["messageid"],
                batteryPower, loadPower, solarPower, myTEST, myTESTDescription)
